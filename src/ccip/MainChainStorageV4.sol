...
contract MainChainStorageV4 is Initializable, ProposableOwnableUpgradeable {
    ...
    mapping(address => uint8) public tokenDexID;
    mapping(uint8 => address) public dexIDToRouter;
    mapping(uint8 => address) public dexIDToFactory;
    ...
    function setUniwapRouterV2(address _swapRouter) public onlyOwner {
        swapRouterV2 = IUniswapV2Router02(_swapRouter);
    }

    function setUniswapFactoryV2(address _factory) public onlyOwner {
        factoryV2 = IUniswapV2Factory(_factory);
    }

    function setUniwapRouterV3(address _swapRouter) public onlyOwner {
        swapRouterV3 = ISwapRouter(_swapRouter);
    }

    function setTokenToDexID(address _token, uint8 _dexId) public onlyOwner {
        tokenDexID[_token] = _dexId;
    }
    ...
    function getSwapRouterV3(address _token) public view returns (ISwapRouter) {
        uint8 dexId = tokenDexID[_token];
        if (dexId == 0) {
            return swapRouterV3;
        } else {
            return ISwapRouter(dexIDToRouter[dexId]);
        }
    }
     ...
     */
    function getMinAmountOut(address[] memory path, uint24[] memory fees, uint256 amountIn, address _token)
        public
        view
        returns (uint256)
    {
        uint256 amountOut = getAmountOut(path, fees, amountIn, _token);
     ...
     */
    function getAmountOut(address[] memory path, uint24[] memory fees, uint256 amountIn, address _token)
        public
        view
        returns (uint256 finalAmountOutValue)
    {
        ...
        if (amountIn > 0) {
            if (fees.length > 0) {
                finalAmountOut = estimateAmountOutWithPath(path, fees, amountIn, _token);
            } else {
    ...
    function estimateAmountOutWithPath(address[] memory path, uint24[] memory fees, uint256 amountIn, address _token)
        public
        view
        returns (uint256 amountOut)
    {
        uint8 tokenDexId = tokenDexID[_token];
        address factory = tokenDexId == 0 ? address(factoryV3) : dexIDToFactory[tokenDexId];
        uint256 lastAmount = amountIn;
        for (uint256 i = 0; i < path.length - 1; i++) {
            lastAmount = IPriceOracle(priceOracle)
                .estimateAmountOut(address(factory), path[i], path[i + 1], uint128(lastAmount), fees[i]);
        }
