...
contract MainChainBalancer is Initializable, ProposableOwnableUpgradeable, PausableUpgradeable {
    ...
    function requestRebalance(address _indexToken, uint256 _targetPortfolioValue, address _usdcAddress, uint256 _dedicatedUSDCAmount) public whenNotPaused onlyOwnerOrOperator {
        ...
        if(_dedicatedUSDCAmount > 0) {
            ...
            uint256 reweightExtraPercentage = ((_targetPortfolioValue - portfolioValue) * 100e18) / _targetPortfolioValue;
    ...
    function _firstReweightSwaps(
        ...
        if (chainValue*100e18/indexFactoryBalancer.getGlobalPortfolioValueByProviderNonce(1, nonce) > _oracleChainSelectorTotalShares) {
    ...
    function _swapTokensToWETHFirstRebalance(
        ...
        chainCurrentRealShare = (mainChainStorage.chainValueByNonce(_nonce, chainSelector) * 100e18) / indexFactoryBalancer.getGlobalPortfolioValueByProviderNonce(1, _nonce);
    ...
    function _swapWETHToTokensForFirstRebalance(
        ...
        uint256 oracleChainSelectorTotalShares = functionsOracle.getOracleChainSelectorTotalShares(_indexToken, _latestOracleCount, chainSelector);
    ...
    function _updateExtraValuesMapping(
        ...
        uint256 targetChainValue = (indexFactoryBalancer.getGlobalPortfolioValueByProviderNonce(1, nonce) * oracleChainSelectorTotalShares) / 100e18;
    ...
    function secondReweightAction(address _indexToken) public whenNotPaused onlyOwnerOrOperator {
        ...
        for (uint256 i = 0; i < chainSelectors.length; i++) {
            ...
            if ((chainValue * 100e18) / indexFactoryBalancer.getGlobalPortfolioValueByProviderNonce(1, nonce) < oracleChainSelectorTotalShares) {
    ...
    function _swapLowerValueCurrentChain(
        ...
        swapVars.swapWethAmount = _swapLowerValueCurrentChainToWETH(
            _indexToken, mainChainStorage.vault(), chainSelector, chainSelectorCurrentTokensCount, portfolioValue, swapVars.chainValue
        );

        uint256 targetChainValue = (indexFactoryBalancer.getGlobalPortfolioValueByProviderNonce(1, nonce) * oracleChainSelectorTotalShares) / 100e18;
        // uint256 negativePercentage = targetChainValue > portfolioValue ? ((targetChainValue - portfolioValue) * 100e18) / portfolioValue : 0;
        uint256 negativePercentage = (targetChainValue - swapVars.chainValue) * 100e18 / indexFactoryBalancer.getGlobalPortfolioValueByProviderNonce(1, nonce);
        ...
        mainChainStorage.increaseConsumedExtraWethByNonce(nonce, mainChainStorage.extraWethByNonce(nonce) - extraWethAmount);
        swapVars.swapWethAmount += extraWethAmount;
        _swapLowerValueCurrentChainFromWETH(
            _indexToken, mainChainStorage.vault(), chainSelector, swapVars.swapWethAmount, oracleChainSelectorTotalShares
        );
    }
    ...
    function _calculateExtraAmountForLowerValue(
        ...
        uint256 targetChainValue = (indexFactoryBalancer.getGlobalPortfolioValueByProviderNonce(1, _nonce) * _oracleChainSelectorTotalShares) / 100e18;
        // uint256 negativePercentage = _oracleChainSelectorTotalShares - chainCurrentRealShare;
        uint256 negativePercentage = (targetChainValue - _chainValue) * 100e18 / indexFactoryBalancer.getGlobalPortfolioValueByProviderNonce(1, _nonce);
        ...
        mainChainStorage.increaseConsumedExtraWethByNonce(_nonce, mainChainStorage.extraWethByNonce(_nonce) - extraWethAmount);
