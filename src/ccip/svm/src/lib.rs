...
pub mod example_ccip_receiver {
    ...
    /// ccip_receive is called by the CCIP Offramp program via CPI
    pub fn ccip_receive(_ctx: Context<CcipReceive>, message: Any2SVMMessage) -> Result<()> {
        // verify that the caller is the router's external execution PDA
        require!(
            ctx.accounts.state.is_router(ctx.accounts.authority.key()),
            CcipReceiverError::OnlyRouter
        );
