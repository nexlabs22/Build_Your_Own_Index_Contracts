...
import {IRouterClient} from "contracts-ccip/contracts/interfaces/IRouterClient.sol";
...
import {Client} from "contracts-ccip/contracts/libraries/Client.sol";
import {Internal} from "contracts-ccip/contracts/libraries/Internal.sol";
...
contract MockRouter3 is IRouter, IRouterClient {
    ...
    // execute the message
    function executeMessage(uint256 _messageCount) public {
        ...
        if (!messageExecuted[_messageCount]) {
            ...
            (bool success, bytes memory retData,) = _routeMessage(
                executableMsg, messageInfo.gasForCallExactCheck, messageInfo.gasLimit, messageInfo.receiver
            );
    ...
    function getFee(uint64, Client.EVM2AnyMessage memory) public view override returns (uint256) {
