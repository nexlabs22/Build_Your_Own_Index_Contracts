...
/// @custom:oz-upgrades-from FunctionsOracleV4
contract FunctionsOracleV5 is Initializable, FunctionsClient, ConfirmedOwner {
    ...
    function _initMainCurrentData(address indexToken, address token, uint256 marketShares) internal {
    ...
    function updateOnlyPathAndFee(address _token, address[] memory _fromETHPath, uint24[] memory _fromETHFees)
        public
        onlyOwnerOrOperator
    {
        require(_fromETHPath.length == _fromETHFees.length + 1, "Invalid input arrays");
    ...
    function getOracleProviderIndexes(address indexToken, uint256 oracleFilledCount_)
        public
        view
        returns (uint64[] memory)
    {
    ...
    function getOracleProviderIndexTotalShares(address indexToken, uint256 oracleFilledCount_, uint64 providerIndex)
        public
        view
        returns (uint256)
    {
        return oracleData[indexToken][oracleFilledCount_].oracleProviderIndexTotalShares[providerIndex];
    }
