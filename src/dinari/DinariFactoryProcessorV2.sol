    ...
    ReentrancyGuardUpgradeable
{
    ...
    function checkMultical(uint256 _requestId) public view returns (bool) {
        ...
        DinariStorage.ActionInfo memory actionInfo = dinariStorage.getActionInfoById(indexToken, _requestId);
        if (actionInfo.actionType == 1) {
            return dinariStorage.checkIssuanceOrdersStatus(indexToken, actionInfo.nonce);
        } else if (actionInfo.actionType == 2) {
            return dinariStorage.checkRedemptionOrdersStatus(indexToken, actionInfo.nonce);
        }
        return false;
    }
    ...
    function multical(uint256 _requestId) public {
        ...
        DinariStorage.ActionInfo memory actionInfo = dinariStorage.getActionInfoById(indexToken, _requestId);
        if (actionInfo.actionType == 1) {
            completeIssuance(indexToken, actionInfo.nonce);
        } else if (actionInfo.actionType == 2) {
            completeRedemption(indexToken, actionInfo.nonce);
        }
    }
}
