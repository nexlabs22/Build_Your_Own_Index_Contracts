...
contract UpgradeOrderManager is Script {
    function run() external {
        ...
        if (keccak256(bytes(targetChain)) == keccak256("sepolia")) {
            proxyAddress = vm.envAddress("SEPOLIA_ORDER_MANAGER_PROXY_ADDRESS");
        } else if (keccak256(bytes(targetChain)) == keccak256("arbitrum_mainnet")) {
            proxyAddress = vm.envAddress("ARBITRUM_ORDER_MANAGER_PROXY_ADDRESS");
        } else {
            revert("Unsupported target chain");
        }

        Upgrades.upgradeProxy(proxyAddress, "OrderManagerV4.sol", "", owner);
