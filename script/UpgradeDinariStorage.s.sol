...
contract UpgradeDinariStorage is Script {
    function run() external {
        ...
        if (keccak256(bytes(targetChain)) == keccak256("sepolia")) {
            proxyAddress = vm.envAddress("SEPOLIA_DINARI_STORAGE_PROXY_ADDRESS");
        } else if (keccak256(bytes(targetChain)) == keccak256("arbitrum_mainnet")) {
            proxyAddress = vm.envAddress("ARBITRUM_DINARI_STORAGE_PROXY_ADDRESS");
        } else {
