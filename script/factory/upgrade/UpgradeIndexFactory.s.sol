...
contract UpgradeIndexFactory is Script {
    function run() external {
        ...
        if (keccak256(bytes(targetChain)) == keccak256("sepolia")) {
            proxyAddress = vm.envAddress("SEPOLIA_INDEX_FACTORY_PROXY_ADDRESS");
        } else if (keccak256(bytes(targetChain)) == keccak256("arbitrum_mainnet")) {
            proxyAddress = vm.envAddress("ARBITRUM_INDEX_FACTORY_PROXY_ADDRESS");
        } else {
            revert("Unsupported target chain");
        }

        Upgrades.upgradeProxy(proxyAddress, "IndexFactoryV3.sol", "", owner);
